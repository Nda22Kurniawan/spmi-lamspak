    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    {{-- Ubah Judul Card --}}
                    <h4 class="card-title">Tambah Sekretaris Program Studi</h4>
                    ...
                    <form action="/users/store" method="post">
                        ...
                        <div class="form-group">
                            ...
                            {{-- PENTING: Value role diganti menjadi Sekretaris Program Studi --}}
                            <input type="text" name="role" class="form-control" aria-describedby="helpId"
                                value="Sekretaris Program Studi" hidden required>
                        </div>

                        <div class="form-group">
                            <label for="">Program Studi</label>
                            <select class="form-control" name="prodi_kode">
                                {{-- Loop data prodi (pastikan dikirim dari Controller) --}}
                                @foreach ($prodi as $i)
                                    <option value="{{ $i->kode }}">
                                        {{ $i->name }} - <b>({{ $i->kode }})</b>
                                    </option>
